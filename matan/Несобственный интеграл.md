...
**Теорема**
Пусть $u, v$ - непр. на $[a, \omega)$ и кусочно непрерывно дифференцируемы на $[a, \omega)$, и пусть $\exists\lim\limits_{b\to \omega} u(b)v(b)$. Тогда $\int\limits_a^\omega u\,dv = uv\bigg | ^\omega_a - \int\limits_a^\omega v\,du$. Нужно ещё предположить, что эти интегралы сходятся.

**Доказательство**

> [!error] Доказательства не будет
>  ПУЛЛ РЕКВЕСТ СЮДА ЗАДВИНЬТЕ НУ ПОЖАЛУЙСТА
